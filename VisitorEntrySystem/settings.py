...
ROOT_URLCONF = 'VisitorEntrySystem.urls'

# settings.py
SITE_URL = config('SITE_URL')  # Replace with your domain

# settings.py for email.verification
EMAIL_BACKEND = config('EMAIL_BACKEND')                             # Use SMTP for sending emails
EMAIL_HOST = config('EMAIL_HOST')                                   # SMTP server for Gmail (use your email provider's server)
EMAIL_PORT = config('EMAIL_PORT', default=False, cast=int)          # Port for TLS (secure connection)
...
WSGI_APPLICATION = 'VisitorEntrySystem.wsgi.application'
